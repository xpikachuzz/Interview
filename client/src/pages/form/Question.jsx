...
export const Question = ({id}) => {
    ...
    const {result, error, loading, setLoading, setError} = useFetch(() => 
        supabase.from("Question").select("*").eq(`question_no`, questionId).eq("form_id", formId)
    )

    // Check user has submitted answer for previous question

    // Check the user has already submitted answer for this question
    ...
    async function submitAnswer() {
        ...
        if (nextResult.error) {
            setError("Can't check if there is a next question. Try again later.")
            return
        }
        ...
        <div className="px-10 py-10 w-7xl">
            {
                data.map(
                    ({question}) => (
                        <div key={questionId} className="flex flex-col justify-start items-start gap-5">
                            <h1 className="text-5xl font-bold pb-2">{question}</h1>
                            <textarea ref={answerRef} className='bg-slate-100 w-3/4 p-1 max-w-4xl max-2xl:border-2 '/>
                            <button
